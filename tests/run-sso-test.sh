...
# Single SSH session for both perm changes — saves one sudo prompt
# when running over SSH from the Mac.
run_remote "sudo yunohost user permission remove ${APP}.main visitors && sudo yunohost user permission add ${APP}.main all_users"
